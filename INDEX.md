...
it is the only graphical surface this project has. Later steps turn that tree
into a sidebar webview, make projects labels a meeting carries zero or more of,
and push the notes of a tagged meeting into each of its projects' Google Docs.
...
| `referat/projects.py`    | 14         | `<meetings_dir>/projects.json` — one entry per project holding an id, a display name, zero or more Google Doc references, a one-line `description` and a `glossary` — plus the tag verbs that write `meta.json`'s `tags`. A project is a label a meeting carries, not a container it sits in, and the id is fixed at creation so a rename touches this file alone. Machine-written and rewritten whole, which is why it is not in `config.py`, whose promise never to write TOML back is about `config.toml`. `glossary` is the one key here that something outside the digests reads: `hotwords.py` merges every project's, and `/cleanup` normalizes a tagged meeting's notes against those of its tags. |
