...
INDEX_MD = "INDEX.md"
PEOPLE_MD = "PEOPLE.md"
"""Everybody Referat knows by name, generated beside `INDEX.md` for the cleanup pass.

Names and nothing else — see :func:`referat.index.render_people`. It lives in the
meetings folder, which may be synced, so it may never carry an email or anything
out of `.voices/` beyond the two spellings a transcript already uses."""
...
RECAPS_DIR = "recaps"
"""`/recap`'s output: one `<project-id>.md` briefing on where that project stands.

The day summary's shape turned ninety degrees — one project across every meeting
carrying its tag, rather than one day across every project — and laid out the
same way: one flat generated folder with one Markdown file per thing, at the
meetings folder root and never inside a meeting, because a recap is a property
of the project. Keyed by the id and not the name, since a rename never moves an
id. Excluded by name in :func:`list_meeting_dirs` exactly as `days/` is.

While a pass runs, the bundle it reads sits beside its output as
`<project-id>.bundle.md` and is removed afterwards — see
:func:`referat.recap.bundle_path`.
"""
...
def list_meeting_dirs(meetings_dir: Path) -> list[Path]:
    ...
    :data:`VOICES_DIR`, :data:`DAYS_DIR` and :data:`RECAPS_DIR` are named here
    even though none of them holds a `meta.json` and so none could pass the gate
    anyway. That is deliberate: a rule belongs where somebody would break it, and
    the thing that would break it is somebody one day relaxing the `meta.json`
    test — at which point the voiceprints, the day summaries and the recaps
    would all start being listed as meetings, and only one of those mistakes is
    easy to notice.
    """
    if not meetings_dir.is_dir():
        return []
    skip = {VOICES_DIR, DAYS_DIR, RECAPS_DIR}
    dirs = [
        p
